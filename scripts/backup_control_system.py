...
#   Section manage variable
        ...
        self.message_addidition_force = nm.control_command()
        self.addition_force = nm.control_command()
...
#   End part function for activate and start part callback
    ...
    def load_addition_force( self ):
        with self.lock_addition_force():
            # Mean you have to get new data
            if self.addition_force.header.stamp != self.message_addidition_force.header.stamp : 
                for run in range( 0 , 6 ):
                    if self.message_addidition_force.mask[ run ]
                        self.addition_force.data[ run ] = self.message_addidition_force.data[ run ]
                    else:
                        self.addition_force.data[ run ] = 0
            else:
                None
